...
class TrustGdycxtSpider(scrapy.Spider):
    name = "trust17_gdycxt_spider"
    allowed_domains = ["gdycxt.com"]

    start_urls = (
        'http://www.gdycxt.com/cn/page/41.html',
    )
    ...
    def parse_item(self, response):
        ...
        for tr in trs:
            tds = tr.find_all('td')
            if len(tds) != 5 or tds[0].text.strip() == '产品名称':
                continue
